#This is a very simple Breakout game
import turtle as tr
...
#Moving the paddle with a listener
screen.listen()
...
def check_collision_with_walls():
    ...
    #Detect a collision with the upper wall
    if ball.ycor() > 270:
        ball.bounce(x_bounce=False, y_bounce=True)
        return

    #Detect a collision with the bottom wall
    # In this case, user failed to hit the ball
    # thus the player loses. The game resets.
    if ball.ycor() < -280:
        ball.reset()
...
def check_collision_with_paddle():
    ...
    #Check if the ball's distance (from its middle)
    # from the paddle (from its middle) is less than the
    # width of paddle and that the ball is below a certain
    # coordinate to detect their collisions.
    if ball.distance(paddle) < 110 and ball.ycor() < -250:
        ...
        #If the paddle is on the left side of the screen
            ...
            if ball_x < paddle_x:
                #If ball hits on the left side it
